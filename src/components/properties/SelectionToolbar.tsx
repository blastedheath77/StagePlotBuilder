...
const TINT_COLORS = [
  { name: 'Default', value: undefined, bg: 'bg-studio-700' },
...
export const SelectionToolbar: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center gap-2 animate-in fade-in slide-in-from-bottom-3 duration-200">
      ...
      {showColorPicker && (
        <div className="flex items-center gap-1.5 p-2 bg-studio-900/95 border border-studio-700 rounded-xl shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95 duration-150">
          <span className="text-[10px] font-mono text-studio-400 uppercase pr-1">Tint:</span>
          {TINT_COLORS.map((c) => (
            <button
              key={c.name}
              type="button"
      ...
      <div className="flex items-center gap-2 px-3.5 py-2 bg-studio-900/95 border border-studio-700/80 rounded-xl shadow-2xl backdrop-blur-md text-studio-200">
        {/* Selection Info */}
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2 py-1 rounded-lg transition-colors text-xs font-medium ${
            showColorPicker
              ? 'bg-studio-800 text-sky-400'
              : 'hover:bg-studio-800 text-studio-300 hover:text-white'
          }`}
        >
